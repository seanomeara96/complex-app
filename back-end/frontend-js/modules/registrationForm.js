...
export default class RegistrationForm {
    ...
    emailAfterDelay(){
        ...
        if(!this.email.errors){
            axios.post("/doesEmailExist", {_csrf: this._csrf, email: this.email.value}).then((response)=>{
                if(response.data){
                    this.email.isUnique = false
                    this.showValidationError(this.email,"that email is taken bro")
                }else{
    ...
    usernameImmediately(){
        ...
        if(this.username.value.length > 30){
            this.showValidationError(this.username, "Username cannot exceed 30 chracters.")
        }
    ...
    usernameAfterDelay(){
        ...
        if(!this.username.errors){
            //axios post 2 arguments
            //1st is th url we want to send the request to
            //2nd is an object of data we want to send to the server
            axios.post('/doesUsernameExist', {_csrf: this._csrf, username: this.username.value}).then((response)=>{
                if(response.data){
                    this.showValidationError(this.username, "Haha, looks like someone got there first loser")
                    this.username.isUnique = false
                }else{
